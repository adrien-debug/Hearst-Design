...
class AuthService {
  ...
  async login(email, password, projectId = null) {
    try {
      // Récupérer l'utilisateur avec son tenant_id
      ...
      // Vérifier que l'utilisateur a un tenant (multi-tenant)
      if (!user.tenant_id) {
      ...
      if (projectId) {
        const { data: access } = await this.supabase
          ...
          .eq('tenant_id', user.tenant_id); // Scoper par tenant
        ...
        // Récupérer tous les accès projets du tenant
        const { data: access } = await this.supabase
          .from('user_project_access')
          .select('*')
          .eq('user_id', user.id)
      ...
      // Générer le token avec tenant_id
      const token = jwt.sign(
        {
          id: user.id,
          email: user.email,
          role: user.role,
          tenant_id: user.tenant_id, // NOUVEAU: inclure tenant_id dans JWT
      ...
      return {
        ...
        user: {
          ...
          name: user.name,
          role: user.role,
          tenant_id: user.tenant_id, // NOUVEAU: inclure dans la réponse
   ...
   */
  async hasProjectAccess(userId, projectId, tenantId = null) {
